...
proc makePassword*(password, salt: string, comparingTo = ""): string =
  ...
  let bcryptSalt = if comparingTo != "": comparingTo else: genSalt(10)
  result = bcrypt.hash(password,bcryptSalt)
  
proc makePhpbbPassword*(password, hash: string): string =
  ## password_hash from PHP.
  var salt = hash[0 .. 29]
  salt[2]='a'
  var newhash = bcrypt.hash(password,salt)
  newhash[2]='y'
  result = newhash
